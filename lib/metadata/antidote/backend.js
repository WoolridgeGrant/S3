...
let antidoteTransac = require('./antidote_transaction');
    ...
    constructor() {
		...
		for(var port in config.antidote){
			if(config.antidote.hasOwnProperty(port)){
				...
				this.antidote[i] = antidoteTransac.connect(port, config.antidote[port]);
...
*/
    ...
    createBucket(bucketName, bucketMD, log, cb) {
		...
		var antidotedb = this.antidote[0];
		antidoteTransac.setBucket(antidotedb, bucketName);

		antidoteTransac.startTx(antidotedb, (tx) => {
			this.getBucketAttributes(bucketName, log, tx, (err, bucket) => {
				...
				antidotedb.defaultBucket = `storage/${bucketName}`;
				let bucket_MD = antidotedb.map(`${bucketName}/md`)
				const bucketMap = []
				Object.keys(bucketMD).forEach(key => {
					bucketMap.push(bucket_MD.register(key).set(bucketMD[key]));
				});
				antidoteTransac.updateSet(tx, bucketMap, 'c', (tx) => {
					antidoteTransac.commitTx(tx, () => {});
					return cb();
				});
			});
		});
    ...
    getBucketAttributes(bucketName, log, tx, cb) {
        ...
        this.antidote[this.indice_site].defaultBucket = `storage/${bucketName}`;
        let bucket_MD = this.antidote[this.indice_site].map(`${bucketName}/md`)
        antidoteTransac.readMap(tx, bucket_MD, (tx, result) => {
			console.log("Dans callback readMap\n");
