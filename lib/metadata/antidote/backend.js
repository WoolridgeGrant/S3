...
class AntidoteInterface {
    ...
    constructor() {
		this.antidote = [];
		var i = 0;
		AntidoteInterface.nb_sites = config.antidote.length;
		for(var port in config.antidote){
			if(config.antidote.hasOwnProperty(port)){
				...
				//Probleme au niveau des noms de machine et des ports 
				//voir les ports et noms de machine utilises par antidote
	...
	findACL(bucketName){
		...
		id = Math.abs(id);
		var indice_site = id%AntidoteInterface.nb_sites;
    ...
    putBucketAttributes(bucketName, bucketMD, log, cb) {
        ...
        this.getBucketAttributes(bucketName, log, err => {
            ...
            this.antidote[indice_site].update(
                bucketMap
            ).then( (resp) => {
                return cb();
            });
