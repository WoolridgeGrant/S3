...
class Config {
    ...
    _getConfig() {
        ...
        this.metadataDaemon = { host: 'localhost', port: 9990 };
        if (config.metadataDaemon) {
            if (config.metadataDaemon.host !== undefined &&
                config.metadataDaemon.port !== undefined) {
                assert.strictEqual(
                ...
                assert(config.metadataDaemon.host === undefined &&
                       config.metadataDaemon.port === undefined,
                       'bad config: you must specify both host and ' +
                       'port of metadata daemon or neither of them');
            }
        }
