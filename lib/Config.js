...
class Config {
    ...
    _getConfig() {
        ...
        if (config.antidote) {
			...
			for(var i = 0; i < config.antidote.length; i++){
				if(config.antidote[i].port !== undefined && config.antidote[i].host !== undefined)
					this.antidote[config.antidote[i].port] = config.antidote[i].host;
			}
			/*
